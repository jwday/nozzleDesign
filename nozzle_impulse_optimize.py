...
fig1, ax = plt.subplots(2, 1, figsize=figsize, dpi=dpi, sharex='col')

ax[0].plot(list_of_expansion_ratios, [x*1000 for x in list_of_cumulative_impulses], color='#ff7f0e', linestyle='-', linewidth=linewidth)
ax[0].set_title('Required Drill Depth for Varying Expansion Ratio')
ax[0].set_ylabel('Net Impulse, mN-s', color='#413839', fontsize=fontsize)
ax[0].tick_params(colors='#413839')
ax[0].grid(which='major', axis='both', linestyle='--')
box0 = ax[0].get_position()
ax[0].set_position([box0.x0 + box0.width * 0.05, box0.y0 + box0.height * 0.05, box0.width, box0.height])

ax[1].plot(list_of_expansion_ratios, [x*1000 for x in list_of_drill_depths], color='#ff7f0e', linestyle='-', linewidth=linewidth)
ax[1].set_xlabel('Expansion Ratio, \u03B5', color='#413839', fontsize=fontsize)
ax[1].set_ylabel('Drill Depth, mm', color='#413839', fontsize=fontsize)
ax[1].tick_params(colors='#413839')
ax[1].grid(which='major', axis='both', linestyle='--')
box1 = ax[1].get_position()
ax[1].set_position([box1.x0 + box1.width * 0.05, box1.y0 + box1.height * 0.05, box1.width, box1.height])

fig1.align_ylabels()
# plt.title('Drill Depth vs. Expansion Ratio')
# plt.tight_layout()
plt.show()