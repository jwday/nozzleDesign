...
pulse_startstop_times = np.linspace(0, max_time, num_thrust_pulses*2)
np.append(pulse_startstop_times, [60])

# Assume pulses decrease linearly in thrust over time (triangular pulse)
thrust_pulse_duration = max_time/(2*num_thrust_pulses)  # Assuming plenum refil is equal in time to pulse duration
max_pulse_thrust = 2*thrust_pulse_impulse/thrust_pulse_duration  # The "height" of the pulse triangle

pulse_thrust = []
for i in t:
    n = 0
    if n < 2*num_thrust_pulses-1:
        if i > pulse_startstop_times[n] and i < pulse_startstop_times[n+1]:
            pulse_thrust.append( i*(-max_pulse_thrust/thrust_pulse_duration) + max_pulse_thrust*(2*n+1))
        elif i > pulse_startstop_times[n+1] and i < pulse_startstop_times[n+2]:
