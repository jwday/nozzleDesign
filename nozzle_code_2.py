...
def nozzle(P_t, T_t, P_amb, d_throat, expansion_ratio, half_angle, gas_type):
	...
	M_exit_behindshock = np.sqrt( (1 + L*M_crit_sup**2)/(k*M_crit_sup**2 - L) )  # Should be < 1 by virtue of properties of normal shocks
	PR_exit_shock = 1/(np.sqrt( S*(P**Q)*M_exit_behindshock*(1 + L*M_exit_behindshock**2) ))  # (P_amb/P_t)_exit_shock  
	...
	if P_amb/P_t < 1 and P_amb/P_t >= PR_crit_sub:
		...
		# (L*M_exit^2 + 1)**W = (P_t/P_amb)
		# (L*M_exit^2 + 1)**-W = (P_amb/P_t)
	...
	# Shock somewhere in the flow (Overexpansion)
		...
		if P_amb/P_t > PR_exit_shock:
			# Shock occurs inside the nozzle. Will not occur in vaccuum.
	...
	if P_amb/P_t < 1 and P_amb/P_t >= PR_crit_sub:  # If entirely subsonic...
		def objective2(X):
