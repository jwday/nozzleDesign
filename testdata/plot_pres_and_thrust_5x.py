...
for T_t in list_of_T_ts:
	...
	# Sim results
	df = pd.DataFrame(data={'Pressure (psig)': [(x/6894.76)-14.7 for x in list_of_P_ts], 'Temperature (K)':[T_t for x in list_of_P_ts], 'Thrust (mN)': [x*1000 for x in list_of_thrusts], 'mdot (g/s)': list_of_mdots})
...
thrust_means = avg_of_data.groupby('Set Point').mean().drop(['Trial'], axis=1).sort_values(['Pressure (psig)'], ascending=False).reset_index()
thrust_stdevs = avg_of_data.groupby('Set Point').std().drop(['Trial'], axis=1).sort_values(['Pressure (psig)'], ascending=False).reset_index()
...
data1.insert(8, "P act_est (psig)", [x-6.5 for x in data1["P init (psig)"]], True)
data1.insert(11, "dM", [round(x,2) for x in data1['M final (g)']-data1['M init (g)']])
data1.insert(12, "dM err", [round(np.sqrt(0.02),2) for x in data1["dM"]])
...
axs[1].set_xlabel(r'Time, $sec$', fontsize=8)
...
for i, T_t in enumerate(list_of_T_ts):
	...
	axs[0].plot("Pressure (psig)", "Thrust (mN)", data=sim_results.loc[sim_results['Temperature (K)'] == T_t], label=label, linestyle=list(lines.lineStyles.keys())[i])
sns.scatterplot(ax=axs[0], x="Pressure (psig)", y="Thrust (mN)", data=thrust_means, s=6**2, marker='s', label='Experimental')
...
for i, T_t in enumerate(list_of_T_ts):
	label = 'Isentropic (' + str(T_t) + ' K)'
	axs[1].plot("Pressure (psig)", "mdot (g/s)", data=sim_results.loc[sim_results['Temperature (K)'] == T_t], label=label, linestyle=list(lines.lineStyles.keys())[i])
sns.scatterplot(ax=axs[1], x="P act_est (psig)", y="dM/dt", data=mdot_means, s=6**2, marker='s', label='Experimental')


axs[0].set_ylabel(r'Thrust, $mN$', fontsize=8)
axs[1].set_ylabel(r'$\dot{m}$, $g/s$', fontsize=8)
axs[1].set_xlabel(r'Pressure, $psig$', fontsize=8)

axs[0].set_ylim(bottom=0, top=300)
axs[1].set_ylim(bottom=0, top=0.8)
axs[1].set_xlim(left=0, right=110)
